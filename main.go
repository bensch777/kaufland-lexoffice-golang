...
func main() {
    ...
    writer.Write([]string{"Auftraggeber", "Buchungsdatum", "Verwendungszweck", "Betrag"})
    for _, record := range records {
        if strings.Contains(record[1], "Verkaufserlös") && !strings.Contains(record[1], "Storno") {
            fmt.Println("Kaufland: "+record[3]+" "+record[8]+" "+record[10])
            writer.Write([]string{"Kaufland", ""+record[0]+"", "KL-VERKAUFSERLÖS-"+record[3]+" - ", record[8]})
